...
var configStore = make([]resolvedConfig, 0)
var configSet = make([]bool, 0)
...
func get_plugin_info() uint32 {
	return setSharedBytes([]byte(`{"name":"case-syntax-slice-store","version":"0.0.0","configKey":"isolation","helpUrl":"https://example.com","configSchemaUrl":""}`))
}
...
func register_config(configID uint32) {
	_ = takeSharedBytes()
	id := int(configID)
	if id >= len(configStore) {
		newLen := id + 1
		newStore := make([]resolvedConfig, newLen)
		copy(newStore, configStore)
		configStore = newStore

		newSet := make([]bool, newLen)
		copy(newSet, configSet)
		configSet = newSet
	}
	configStore[id] = resolvedConfig{IndentWidth: 2, UseTabs: false}
	configSet[id] = true
}
...
func get_resolved_config(configID uint32) uint32 {
	id := int(configID)
	if id >= 0 && id < len(configSet) && configSet[id] {
		_ = configStore[id]
	}
