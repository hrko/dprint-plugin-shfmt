...
func get_plugin_info() uint32 {
	return setSharedBytes([]byte(`{"name":"case-syntax-json-resolved-marshal-const","version":"0.0.0","configKey":"isolation","helpUrl":"https://example.com","configSchemaUrl":""}`))
}
...
func get_resolved_config(_ uint32) uint32 {
	bytes, err := json.Marshal(resolvedConfig{IndentWidth: 2, UseTabs: false})
	if err != nil {
		panic(err)
	}
