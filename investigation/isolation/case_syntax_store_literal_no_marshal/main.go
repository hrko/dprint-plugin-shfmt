...
var configStore = map[uint32]resolvedConfig{}
...
func get_plugin_info() uint32 {
	return setSharedBytes([]byte(`{"name":"case-syntax-store-literal-no-marshal","version":"0.0.0","configKey":"isolation","helpUrl":"https://example.com","configSchemaUrl":""}`))
}
...
func register_config(configID uint32) {
	_ = takeSharedBytes()
	configStore[configID] = resolvedConfig{
		IndentWidth: 2,
		UseTabs:     false,
	}
}
