...
var configStore = map[uint32]rawFormatConfig{}
...
func get_plugin_info() uint32 {
	return setSharedBytes([]byte(`{"name":"case-syntax-json-register-typed","version":"0.0.0","configKey":"isolation","helpUrl":"https://example.com","configSchemaUrl":""}`))
}
...
func get_resolved_config(configID uint32) uint32 {
	cfg := configStore[configID]
	if cfg.Plugin.IndentWidth == 0 && !cfg.Plugin.UseTabs {
		return setSharedBytes([]byte(`{"indentWidth":2,"useTabs":false}`))
	}
	bytes, err := json.Marshal(cfg.Plugin)
	if err != nil {
		panic(err)
	}
